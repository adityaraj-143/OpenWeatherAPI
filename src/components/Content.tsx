...
const Content = () => {
  ...
  const date = new Date();
  const [day,month,riseTime,setTime] = functions(details, date)
      ...
      <div className="bg-green-500 p-5">
        <div className="flex">
          <div className="left-0 relative">
            <h1 className=" text-3xl pt-4 pl-8 absolute">{details.weather?.name}</h1>
            <div className="flex right-0 absolute">
              <img
                ...
                alt="climate"
              />
              <p className="pt-6 pr-8 text-xl">{details.weather?.main.temp}°<sup>C</sup></p>
            </div>
            <div >
              <img 
                src={bgImg[0].imgUrl}
          ...
          <div className="w-30 my-auto ml-14  ">
            <div className="p-5 h-28 pt-8 bg-green-300">
              <p>{day}, {date.getDate()}<sup>th</sup> {month}</p>
              <p>{date.getHours()}: {date.getMinutes()}</p>
              <small></small>
            </div>
            <div className="p-5 pt-8 h-28 bg-green-300 mt-16">
              <p>Feels like: { details.weather?.main.feels_like}°<sup>C</sup></p>
        ...
        <div>
          ...
          <div className="flex gap-14">
            <div className="bg-white w-32 h-20 rounded-xl border-slate-400 border-2">
              ...
              <p className="text-center pt-3">{details.forecast && 100*details.forecast?.list[0].pop}%</p>
            </div>
            <div className="bg-white w-32 h-20 rounded-xl border-slate-400 border-2">
              <small className="text-start px-2">Humidity</small>
              <p className="text-center pt-3">{details.weather && details.weather.main.humidity}%</p>
            </div>
            <div className="bg-white w-32 h-20 rounded-xl border-slate-400 border-2">
              <small className="text-start px-2">Wind</small>
              <p className="text-center pt-3">{details.weather && (3.6 * details.weather?.wind.speed).toFixed(2)}km/h</p>
            </div>
            <div className="bg-white w-32 h-20 rounded-xl border-slate-400 border-2">
